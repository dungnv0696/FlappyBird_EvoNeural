...
class Game:
    ...
    def new(self):
        ...
        #region Collect data
        print("Average Fitness List: "+ str(self.average_fitness_list))
        print("Best Fitness List: "+str(self.best_fitness_list))
        print("Generation List: "+str(self.generation_list))
        print()

        '''
        file = open("thehe.txt", "w")
        for i in range(len(self.generation_list)):
            file.write(" "+str(self.generation_list[i]))
        file.close()

        file = open("best_fitness.txt", "w")
        for i in range(len(self.best_fitness_list)):
            file.write(" "+str(self.best_fitness_list[i]))
        file.close()
        '''
    ...
    def events(self):
        for event in pg.event.get():
            if event.type == pg.QUIT:  # An thoat
                raise SystemExit #Thoat game

            if event.type == pg.KEYDOWN and event.key == pg.K_ESCAPE: #An esc de bat dau the he moi
    ...
    def draw(self):
        ...
        if len(self.all_birds) != 0: #update best _score__:
            self.__display_text__("Diem cao nhat: " + str(self.__score__), 10, 50, 20, RED)
            self.__display_text__("So chim con lai " + str(len(self.all_birds)), 10, 30, 20, BLACK)
        ...
        self.__display_text__("The he: " + str(self.generation), 10, 10, 20, BLACK)
        self.__display_text__("The he truoc do:", 10, 70, 20, BLUE)
        self.__display_text__("Best Fitness: " + str(self.best_fitness_list[-1]), 10, 90, 20, BLUE)
        self.__display_text__("Average Fitness: " + str(self.average_fitness_list[-1]), 10, 110, 20, BLUE)
        self.__display_text__("An F1 de xem do thi", 10, 550, 20, BLACK)
        self.__display_text__("An Esc de bat dau the he moi", 10, 570, 20, BLACK)
    ...
    def start_screen(self):
        ...
        while not self.__game_playing__:
            ...
            self.screen.blit(bird_img, [WIDTH/2, HEIGHT/2.5])
            self.__display_text__("Flappy Bird",WIDTH / 3, HEIGHT / 4, 30, BLACK)
            self.__display_text__("An Enter Đe Bat Dau",WIDTH / 6, HEIGHT / 2, 30, BLACK)
            pg.display.flip()
            ...
            for event in pg.event.get():
                ...
                if event.type == pg.KEYDOWN and event.key == pg.K_RETURN: #Ấn Enter Để Bắt Đầu
    ...
    def veBieuDo(self):
        ...
        x = np.asarray(self.generation_list)
        y = np.asarray(self.best_fitness_list)
